...
use log::info;
use uefi::prelude::*;
use uefi::proto::console::text::{Key, Output, ScanCode};
...
///
/// # Arguments
/// * `system_table` - UEFI System Table for console I/O
...
pub fn show_boot_menu(
    ...
    // Initial draw
    draw_menu(system_table, config, selected, remaining_seconds);

    // Main loop
    loop {
        // Check for key press (non-blocking)
        if let Some(key) = read_key_nonblocking(system_table) {
            match key {
        ...
        if remaining_seconds > 0 {
            // Wait 1 second
            system_table.boot_services().stall(1_000_000); // 1 second in microseconds
            remaining_seconds -= 1;
            ...
            system_table.boot_services().stall(50_000); // 50ms
        }
    }
}
...
fn print_string(stdout: &mut Output, s: &str) {
...
fn print_number(stdout: &mut Output, n: u32) {
    let mut buf = [0u16; 12];
    let mut num = n;
    let mut pos = 10;
...
fn read_key_nonblocking(system_table: &mut SystemTable<Boot>) -> Option<Key> {
