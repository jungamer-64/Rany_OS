...
impl From<&SelfTestResults> for SelfTestInfo {
    fn from(results: &SelfTestResults) -> Self {
        Self {
            overall_result: match results.overall {
                TestResult::Pass => 0,
                TestResult::Warning => 1,
                TestResult::Fail => 2,
                TestResult::Skip => 3,
            },
            critical_failures: results.critical_failures,
            warnings: results.warnings,
            tests_run: results.tests.len() as u8,
            _reserved: [0; 4],
        }
    }
}
