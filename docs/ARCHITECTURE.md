...
- Storage/FS required 実行対象: async_ops + async_memfs + cache(core+block) + fs_abstraction + memfs + page + page_cluster_buffer + procfs（full-boot runtime では `posix-compat` 有効）。
