...
1. virtio-blk で borrowed API を実装（最初はコピーでも可）
2. 可能なら DMA 直結パスを優先実装（`IoBuffer::dma_info()` を参照し、必要なら `DmaHandle` でマップ）
...
- ⚠️ **ブロックデバイス（virtio-blk 等）は borrowed API 未対応**: `kernel/src/io/virtio/blk.rs` は `&mut [u8]` を受ける独自 async APIのみで、`ZeroCopyBlockDevice` への橋渡しが必要です。
...
2. PR2: virtio-blk で borrowed API 実装（必要なら DMA 直結） - **高**
