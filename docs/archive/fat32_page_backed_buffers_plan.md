...
- 単体テスト／統合テストを追加し、`cargo test`（host純）と full-boot QEMU required がグリーンになる
...
- 統合: `QEMU_TEST_PROFILE_ONLY=storage cargo test -p qemu-tests fullboot_pr_required -- --exact --nocapture`、QEMU 上の virtio-blk シナリオ
...
4. テスト: `QEMU_TEST_PROFILE_ONLY=storage cargo test -p qemu-tests fullboot_pr_required -- --exact --nocapture` で allocator 注入経路を検証。
