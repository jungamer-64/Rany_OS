...
- MM Wave7 required 配線ガード（Phase A+B+C+D+E+F）: `bash scripts/verify_mm_wave7_required.sh`
...
- MM bench pending 監視サマリ: `target/qemu-logs/kernel-mm-pending-summary.txt`, `target/qemu-logs/kernel-mm-pending-summary.json`
- runtime pending は runtime依存2件（`kernel_net_bridge_zero_copy_integration` / `kernel_bench_framebuffer`）専用の non-blocking 監視
- kernel-mm-pending は MM Wave7 bench-smoke 3件（`bench_enqueue_pool_effect`, `bench_buffer_pool_2m_reuse`, `bench_buffer_pool_1g_reuse`）専用の non-blocking 監視
