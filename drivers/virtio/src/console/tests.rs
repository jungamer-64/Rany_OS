...
fn test_device() -> PackedPciLocation {
    PackedPciLocation::new(0, 0, 0x20, 0)
}
...
fn test_console_write_empty_data() {
    ...
    let vq = unsafe { VirtQueue::new(1, queue_size, desc_ptr, avail_ptr, used_ptr, None, 0) }
        .expect("tx queue");

    let mut dev = VirtioConsoleDevice::new(Box::new(NoopTransport), test_device());
    dev.tx_queue = Some(Arc::new(PoisonLock::new(vq)));
