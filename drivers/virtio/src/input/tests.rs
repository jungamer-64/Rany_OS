// ============================================================================
// drivers/virtio/src/input/tests.rs
// ============================================================================
use alloc::vec::Vec;
...
fn test_device() -> PackedPciLocation {
    PackedPciLocation::new(0, 0, 0x10, 0)
}
...
fn test_virtio_input_device_new_with_device() {
    let device_id = PackedPciLocation::new(0, 0, 0, 0);
    let dev = VirtioInputDevice::new_with_device(Box::new(NoopTransport), device_id);
    assert!(!dev.is_ready());
    assert_eq!(dev.pci_locator, device_id);
}
...
fn test_virtqueue_alloc_free_desc() {
    ...
    let vq = unsafe { VirtQueue::new(0, queue_size, desc_ptr, avail_ptr, used_ptr, None, 0) }
        .expect("virtqueue");
...
fn test_query_config_returns_none_for_zero_size() {
    let dev = VirtioInputDevice::new(Box::new(NoopTransport), test_device());
    // NoopTransport returns 0 for all reads, so the shared core yields an empty config payload.
...
fn test_device_name_returns_none_for_noop() {
    let dev = VirtioInputDevice::new(Box::new(NoopTransport), test_device());
    assert_eq!(dev.device_name(), Some(Vec::new()));
}
