...
impl MscDevice {
    ...
    pub fn prepare_command(
        ...
        CommandBlockWrapper::new(self.next_tag(), data_length, direction_in, lun, command)
    }
    ...
    pub fn parse_inquiry_response(data: &[u8]) -> Option<(String, String, String, bool)> {
        ...
        let vendor =
            alloc::string::String::from(core::str::from_utf8(&data[8..16]).unwrap_or("").trim());

        // Product ID (16 bytes at offset 16)
        let product =
            alloc::string::String::from(core::str::from_utf8(&data[16..32]).unwrap_or("").trim());

        // Revision (4 bytes at offset 32)
        let revision =
            alloc::string::String::from(core::str::from_utf8(&data[32..36]).unwrap_or("").trim());
