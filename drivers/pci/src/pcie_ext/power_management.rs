...
impl PciePowerManager {
    ...
    pub fn set_state(&self, state: PciePowerState) -> PcieResult<()> {
        ...
        let state_bits = match state {
            PciePowerState::D0 => 0,
            PciePowerState::D1 => { if (pmcap & (1 << 9)) == 0 { return Err(PcieError::NotSupported); } 1 }
            PciePowerState::D2 => { if (pmcap & (1 << 10)) == 0 { return Err(PcieError::NotSupported); } 2 }
...
impl PcieMsixController {
    pub fn new(config: &'static PcieConfig, bdf: PcieBdf) -> PcieResult<Self> {
        ...
        Ok(Self { config, bdf, msix_offset: Some(offset), table_size, table_bir, table_offset, pba_bir, pba_offset })
    }
    ...
    pub fn enable(&self) -> PcieResult<()> {
        ...
        self.config.write16(self.bdf, offset as u16 + 2, (msg_ctrl | 0x8000) & !0x4000).ok_or(PcieError::ConfigError)
    }
    ...
    pub fn table_info(&self) -> (u8, u32) { (self.table_bir, self.table_offset) }
    pub fn pba_info(&self) -> (u8, u32) { (self.pba_bir, self.pba_offset) }
}
...
impl HotPlugController {
    pub fn new(config: &'static PcieConfig, bdf: PcieBdf) -> PcieResult<Self> {
        ...
        Ok(Self { config, bdf, pcie_offset: Some(offset), slot_implemented })
    }

    pub fn is_supported(&self) -> bool { self.slot_implemented }
    pub fn is_hotplug_capable(&self) -> bool {
        if !self.slot_implemented { return false; }
        let Some(offset) = self.pcie_offset else { return false; };
        let slot_caps = self.config.read32(self.bdf, offset as u16 + 0x14).unwrap_or(0);
        (slot_caps & (1 << 6)) != 0
    }

    pub fn slot_status(&self) -> PcieResult<u16> {
        if !self.slot_implemented { return Err(PcieError::NotSupported); }
...
impl PcieExtManager {
    ...
    pub fn scan_bus(&self, bus: u8) {
        for device in 0..32 { self.scan_device(bus, device); }
    }

    pub(super) fn scan_device(&self, bus: u8, device: u8) {
        let bdf = PcieBdf::new(bus, device, 0);
        let vendor_id = match self.config.read16(bdf, 0x00) { Some(v) if v != 0xFFFF => v, _ => return };
        ...
        let pcie_device = PcieExtDevice { bdf, vendor_id, device_id, class_code, has_sriov, has_aer, has_msix };
        self.devices.write().unwrap_or_else(|e| e.into_inner()).push(pcie_device);
        let header_type = self.config.read8(bdf, 0x0E).unwrap_or(0);
        if (header_type & 0x80) != 0 {
            for function in 1..8 { self.scan_function(bus, device, function); }
        }
    }
    ...
    pub fn devices(&self) -> Vec<PcieExtDevice> { self.devices.read().unwrap_or_else(|e| e.into_inner()).clone() }
    pub fn find_device(&self, vendor_id: u16, device_id: u16) -> Option<PcieExtDevice> {
        self.devices.read().unwrap_or_else(|e| e.into_inner()).iter().find(|d| d.vendor_id == vendor_id && d.device_id == device_id).cloned()
    }
    pub fn config(&self) -> &'static PcieConfig { self.config }
}
...
pub fn pcie_ext_manager() -> Option<&'static PcieExtManager> { PCIE_EXT_MANAGER.get() }
pub fn pcie_ext_config() -> Option<&'static PcieConfig> { PCIE_EXT_CONFIG.get() }
...
impl AtsController {
    pub fn new(config: &'static PcieConfig, bdf: PcieBdf) -> PcieResult<Self> {
        let offset = config.find_ext_capability(bdf, ext_cap_id::ATS).ok_or(PcieError::CapabilityNotFound)?;
        let cap = Self::read_capability(config, bdf, offset)?;
        Ok(Self { config, bdf, capability: Some(cap), enabled: AtomicBool::new(false) })
    }

    pub(super) fn read_capability(config: &PcieConfig, bdf: PcieBdf, offset: u16) -> PcieResult<AtsCapability> {
        let cap_reg = config.read16(bdf, offset + ats_regs::CAP).ok_or(PcieError::ConfigError)?;
        ...
        let ctrl_reg = config.read16(bdf, offset + ats_regs::CTRL).ok_or(PcieError::ConfigError)?;
        let stu = (ctrl_reg & 0x1F) as u8;
        Ok(AtsCapability { offset, invalidate_queue_depth, page_aligned_request, global_invalidate, relaxed_ordering, stu })
    }
    ...
    pub fn enable_ats(&self, stu: u8) -> PcieResult<()> {
        ...
        let mut ctrl = self.config.read16(self.bdf, offset + ats_regs::CTRL).ok_or(PcieError::ConfigError)?;
        ctrl = (ctrl & !0x1F) | ((stu as u16) & 0x1F);
        ctrl |= 1 << 15;
        self.config.write16(self.bdf, offset + ats_regs::CTRL, ctrl).ok_or(PcieError::ConfigError)?;
    ...
    pub fn capability(&self) -> Option<&AtsCapability> { self.capability.as_ref() }
    pub fn bdf(&self) -> PcieBdf { self.bdf }
}
