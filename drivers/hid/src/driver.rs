...
mod tests {
    ...
    #[test]
    fn test_driver_handle_scancode() {
        let driver = KeyboardDriver::new();

        // Simulate key press (A key = 0x1E)
        driver.handle_scancode(0x1E);
        assert!(driver.has_event());

        // Pop the event
        let event = driver.poll_key_event_internal();
    ...
    #[test]
    fn test_driver_extended_scancode() {
        let driver = KeyboardDriver::new();

        // Simulate extended key (Up arrow = E0 48)
        driver.handle_scancode(0xE0); // Extended prefix
        driver.handle_scancode(0x48); // Up arrow
        assert!(driver.has_event());

        let event = driver.poll_key_event_internal().unwrap();
        assert_eq!(event.key, KeyCode::Up);
    }

    #[test]
    fn test_driver_key_release() {
        let driver = KeyboardDriver::new();

        // Simulate key release (A key released = 0x9E)
