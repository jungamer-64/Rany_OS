...
impl DateTime {
    pub fn to_unix_timestamp(&self) -> i64 {
        ...
        for y in 1970..self.year as i64 {
            if is_leap_year(y as u16) { days += 366; } else { days += 365; }
        }
        let days_in_month = if is_leap_year(self.year) { [0, 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31] } else { [0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31] };
        for m in 1..self.month { days += days_in_month[m as usize] as i64; }
    ...
    pub const fn from_unix_timestamp(timestamp: i64) -> Self {
        ...
        Self { year, month, day, hour, minute, second, day_of_week }
    }
}
...
const fn is_leap_year(year: u16) -> bool { (year % 4 == 0 && year % 100 != 0) || (year % 400 == 0) }
...
impl Rtc {
    ...
    unsafe fn is_update_in_progress() -> bool { unsafe { (Self::read_cmos(regs::STATUS_A) & status_a::UPDATE_IN_PROGRESS) != 0 } }
    const fn bcd_to_binary(bcd: u8) -> u8 { (bcd & 0x0F) + ((bcd >> 4) * 10) }
    const fn binary_to_bcd(bin: u8) -> u8 { ((bin / 10) << 4) | (bin % 10) }
    ...
    pub fn new() -> Self {
        ...
        let periodic_rate = if (3..=15).contains(&rate_sel) { 32768 >> (rate_sel - 1) } else { 0 };
        Self { binary_mode: (status_b & status_b::BINARY_MODE) != 0, hour_24: (status_b & status_b::HOUR_24) != 0, century_register: None, periodic_rate }
    }

    pub fn set_century_register(&mut self, reg: u8) { self.century_register = Some(reg); }
    ...
    pub fn read_datetime(&self) -> DateTime {
        unsafe {
            ...
            let century = if let Some(reg) = self.century_register { Self::read_cmos(reg) } else { 0x20 };
            let (second, minute, hour, day, month, year, century) = if !self.binary_mode {
                (Self::bcd_to_binary(second), Self::bcd_to_binary(minute), {
                    let pm = (hour & 0x80) != 0;
                    let h = Self::bcd_to_binary(hour & 0x7F);
                    if !self.hour_24 && pm { (h % 12) + 12 } else { h }
                }, Self::bcd_to_binary(day), Self::bcd_to_binary(month), Self::bcd_to_binary(year), Self::bcd_to_binary(century))
            } else {
                let pm = (hour & 0x80) != 0;
                let h = hour & 0x7F;
                let adjusted_hour = if !self.hour_24 && pm { (h % 12) + 12 } else { h };
                (second, minute, adjusted_hour, day, month, year, century)
            };
            let full_year = century as u16 * 100 + year as u16;
            DateTime { year: full_year, month, day, hour, minute, second, day_of_week }
        }
    }
    ...
    pub fn write_datetime(&self, dt: &DateTime) {
        unsafe {
            ...
            let (second, minute, hour, day, month, year, century) = if !self.binary_mode {
                (Self::binary_to_bcd(dt.second), Self::binary_to_bcd(dt.minute), Self::binary_to_bcd(dt.hour), Self::binary_to_bcd(dt.day), Self::binary_to_bcd(dt.month), Self::binary_to_bcd((dt.year % 100) as u8), Self::binary_to_bcd((dt.year / 100) as u8))
            } else {
                (dt.second, dt.minute, dt.hour, dt.day, dt.month, (dt.year % 100) as u8, (dt.year / 100) as u8)
            };
            ...
            if let Some(reg) = self.century_register { Self::write_cmos(reg, century); }
    ...
    pub fn set_alarm(&self, hour: u8, minute: u8, second: u8) {
        unsafe {
            let (h, m, s) = if !self.binary_mode {
                (Self::binary_to_bcd(hour), Self::binary_to_bcd(minute), Self::binary_to_bcd(second))
            } else {
                (hour, minute, second)
            };
    ...
    pub fn disable_interrupts(&self) {
        unsafe {
            let status_b = Self::read_cmos(regs::STATUS_B);
            Self::write_cmos(regs::STATUS_B, status_b & !(status_b::PERIODIC_INT | status_b::ALARM_INT | status_b::UPDATE_ENDED_INT));
        }
    }

    pub fn read_interrupt_status(&self) -> InterruptStatus {
        let status_c = unsafe { Self::read_cmos(regs::STATUS_C) };
        InterruptStatus { update_ended: (status_c & 0x10) != 0, alarm: (status_c & 0x20) != 0, periodic: (status_c & 0x40) != 0, irq: (status_c & 0x80) != 0 }
    }
    ...
    pub fn set_frequency(&mut self, hz: u32) -> Result<(), &'static str> {
        let rate = match hz { 8192 => 3, 4096 => 4, 2048 => 5, 1024 => 6, 512 => 7, 256 => 8, 128 => 9, 64 => 10, 32 => 11, 16 => 12, 8 => 13, 4 => 14, 2 => 15, _ => return Err("Unsupported frequency") };
...
pub struct InterruptStatus { pub update_ended: bool, pub alarm: bool, pub periodic: bool, pub irq: bool }
...
pub fn init() {
    ...
    if timestamp > 0 { BOOT_TIMESTAMP.store(timestamp as u64, Ordering::SeqCst); }
    *RTC.lock().unwrap_or_else(|e| e.into_inner()) = Some(rtc);
}

pub fn get_datetime() -> Option<DateTime> {
    RTC.lock().unwrap_or_else(|e| e.into_inner()).as_ref().map(|rtc| rtc.read_datetime())
}

pub fn set_datetime(dt: &DateTime) {
    if let Some(ref rtc) = *RTC.lock().unwrap_or_else(|e| e.into_inner()) { rtc.write_datetime(dt); }
}
...
pub fn get_uptime_seconds() -> u64 { SYSTEM_TIME.load(Ordering::Acquire) }
...
pub fn get_uptime_ms() -> u64 {
    ...
    if let Some(ref rtc) = *RTC.lock().unwrap_or_else(|e| e.into_inner()) {
        ...
        if rate > 0 { let sub_second_ticks = ticks % rate; return (seconds * 1000) + (sub_second_ticks * 1000 / rate); }
    }
    seconds * 1000
}

pub fn get_unix_timestamp_ms() -> i64 { get_unix_timestamp() * 1000 + (get_uptime_ms() % 1000) as i64 }
pub fn check_and_clear_alarm() -> bool { ALARM_TRIGGERED.swap(false, Ordering::Acquire) }
...
pub fn rtc_interrupt_handler() {
    if let Some(ref rtc) = *RTC.lock().unwrap_or_else(|e| e.into_inner()) {
        ...
        if status.periodic {
            ...
            let rate = rtc.periodic_rate as u64;
            if rate > 0 && (ticks + 1) % rate == 0 { SYSTEM_TIME.fetch_add(1, Ordering::Release); }
        }
        if status.alarm { ALARM_TRIGGERED.store(true, Ordering::Release); }
    }
}
