...
use crate::graphics::{Color, Rect, Framebuffer};
use super::types::{ShellState, ShellResources};
...
impl Layout {
    ...
    fn compute(state: &ShellState, res: &ShellResources) -> Self {
        ...
        let completion_y = (!state.temp_fmt_buffer.is_empty()).then(|| {
...
impl<'a, 'b> RenderContext<'a, 'b> {
    ...
    fn with_layout(fb: &'a mut Framebuffer, state: &'b ShellState, res: &'b ShellResources, layout: Layout) -> Self {
        Self { 
            fb, 
            state, 
            res, 
    ...
    fn draw_output_lines(&mut self) {
        let Layout { font_h, fb_height, fb_width, .. } = self.layout;
        ...
        let skip_count = self.state.output_lines.len()
        ...
        self.state.output_lines.iter()
            ...
            .for_each(|(i, line)| {
                ...
                self.res.font.draw_string(self.fb, 0, y, &line.text, line.color, Some(theme.background));
            });
    }

    /// Draws the prompt and current input buffer with culling.
    fn draw_input_line(&mut self) {
        let Layout { input_y, font_h, fb_width, .. } = self.layout;
        ...
        font.draw_string(self.fb, 0, input_y, &self.state.prompt, theme.prompt, Some(theme.background));
        font.draw_string(self.fb, self.state.cached_prompt_end_x, input_y, 
            &self.state.input_buffer.content, theme.input, Some(theme.background));
    }

    /// Draws the blinking text cursor with culling.
    fn draw_text_cursor(&mut self) {
        if !self.state.cursor_visible { return; }

        let Layout { input_y, font_w, font_h, fb_width, .. } = self.layout;
        let cursor_x = self.state.cursor_x();
        if cursor_x >= fb_width { return; }
        ...
        self.res.font.draw_char(self.fb, cursor_x, input_y, c, self.res.theme.background, None);
    }

    /// Draws the completion suggestions line with culling.
    fn draw_completions(&mut self) {
        let Some(y) = self.layout.completion_y else { return; };
    ...
    fn draw_mouse_cursor(&mut self) {
        if !self.state.show_mouse_cursor { return; }
...
impl GraphicalShell {
    ...
    pub fn redraw(&mut self, fb: &mut Framebuffer) {
        self.perform_draw(fb, |shell| {
            RectList::<1>::from_element(Rect::new(0, 0, shell.resources.fb_width, shell.resources.fb_height))
        });
    }
    ...
    fn current_cursor_rect(&self) -> Rect {
        ...
        Rect::new(self.state.cursor_x(), res.input_line_y(), res.font.width(), res.font.height())
    }
