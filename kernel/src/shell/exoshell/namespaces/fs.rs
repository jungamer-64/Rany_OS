    ...
    pub async fn stat(path: &str) -> ExoValue<'static> {
        ...
        match crate::fs::stat_file(path, "/") {
            Ok(attr) => {
                ...
                map.insert(String::from("path"), ExoValue::String(Cow::Owned(path.to_string())));
                ...
                map.insert(String::from("type"), ExoValue::String(Cow::Owned(format!("{:?}", attr.file_type))));
                ExoValue::Map(map)
            }
            Err(e) => ExoValue::Error(format!("{:?}", e)),
        }
    ...
    fn call<'a>(&'a self, method: &'a str, args: &'a [ExoValue]) -> BoxFuture<'a, ExoValue<'static>> {
        Box::pin(async move {
            match method {
                ...
                _ => ExoValue::Error(
                    format!("Unknown method 'fs.{}'\nValid methods: entries, read, stat, mkdir, remove", method)
                ),
            }
        })
    }
