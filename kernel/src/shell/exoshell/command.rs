...
impl ShellCommand for HelpCommand {
    ...
    fn help(&self) -> &str {
        "Display available commands and namespaces"
    }
}
...
impl ShellCommand for ClearCommand {
    ...
    fn execute(&self, _shell: &mut ExoShell, _args: &[ExoValue]) -> ExoResult<ExoValue<'static>> {
        Ok(ExoValue::String(alloc::borrow::Cow::Borrowed(
            "\x1b[2J\x1b[H",
        )))
    }
    fn help(&self) -> &str {
    ...
    fn execute(&self, shell: &mut ExoShell, args: &[ExoValue]) -> ExoResult<ExoValue<'static>> {
        ...
        let entries: Vec<ExoValue<'static>> = history[start..]
            ...
            .map(|(i, cmd)| {
                ...
                map.insert(String::from("index"), ExoValue::Int((start + i) as i64));
                map.insert(
                    String::from("command"),
...
impl ShellCommand for TypeCommand {
    ...
    fn execute(&self, _shell: &mut ExoShell, args: &[ExoValue]) -> ExoResult<ExoValue<'static>> {
        ...
        Ok(ExoValue::String(alloc::borrow::Cow::Owned(String::from(
            type_name,
        ))))
    }
    fn help(&self) -> &str {
        "Show the type of a value. Usage: type <expr>"
    }
}
...
impl ShellCommand for WhoamiCommand {
    ...
    fn execute(&self, _shell: &mut ExoShell, _args: &[ExoValue]) -> ExoResult<ExoValue<'static>> {
        ...
        map.insert(
            String::from("user"),
            ExoValue::String(alloc::borrow::Cow::Borrowed("root")),
        );
        map.insert(String::from("domain_id"), ExoValue::Int(domain_id as i64));
        map.insert(
            String::from("privilege"),
            ExoValue::String(alloc::borrow::Cow::Borrowed("full")),
        );
        Ok(ExoValue::Map(map))
    }
    fn help(&self) -> &str {
        "Show current user/domain identity"
    }
}
...
impl ShellCommand for SetCommand {
    ...
    fn help(&self) -> &str {
        "Set a variable. Usage: set <name> <value>"
    }
}
...
impl ShellCommand for UnsetCommand {
    ...
    fn help(&self) -> &str {
        "Remove a variable. Usage: unset <name>"
    }
}
