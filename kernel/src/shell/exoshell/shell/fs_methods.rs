...
impl ExoShell {
    ...
    pub(super) async fn eval_net_method(
        ...
        match name {
            "config" => NetNamespace::config(&args).await,
            "stats" => NetNamespace::stats(&args).await,
            "arp" => NetNamespace::arp_cache().await,
            "dhcp_state" => NetNamespace::dhcp_state(&args).await,
            ...
            // TCP/UDP接続管理
            "connections" | "netstat" => NetNamespace::netstat().await,
            "tcp" => NetNamespace::tcp_connections().await,
            "udp" => NetNamespace::udp_endpoints().await,
            // インターフェース管理
            "interfaces" | "ifaces" => NetNamespace::interfaces().await,
            "if_up" => NetNamespace::if_up(&args).await,
            "if_down" => NetNamespace::if_down(&args).await,
            // ルーティング
            "routes" => NetNamespace::routes().await,
            "route_add" => NetNamespace::route_add(&args).await,
            "route_del" => NetNamespace::route_del(&args).await,
            // ファイアウォール
            "firewall" => NetNamespace::firewall_status().await,
            "firewall_enable" => NetNamespace::firewall_enable().await,
            "firewall_disable" => NetNamespace::firewall_disable().await,
            "firewall_rules" => NetNamespace::firewall_rules().await,
            "firewall_stats" => NetNamespace::firewall_stats().await,
            "firewall_add" => NetNamespace::firewall_add(&args).await,
            "firewall_remove" => NetNamespace::firewall_remove(&args).await,
            "firewall_clear" => NetNamespace::firewall_clear().await,
            "firewall_policy" => NetNamespace::firewall_policy(&args).await,
            // DNS
            "dns" | "resolve" => NetNamespace::dns_resolve(&args).await,
            // 診断
