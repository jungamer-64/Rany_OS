...
pub fn mpsc_basic_smoke() -> bool {
    ...
    rb.pop() == Some(1) && rb.pop() == Some(2) && rb.pop() == Some(3) && rb.pop().is_none()
}
...
pub fn mpmc_try_operations_smoke() -> bool {
    ...
    if rb.try_push(1).is_err() {
        return false;
    }
    if rb.try_push(2).is_err() {
        return false;
    }
    if rb.try_push(3).is_err() {
        return false;
    }
    if rb.try_push(4).is_err() {
        return false;
    }

    // Full
    if rb.try_push(5).is_ok() {
        return false;
    }
...
pub fn lock_free_index_stack_smoke() -> bool {
    ...
    if stack.push(0).is_err() {
        return false;
    }
    if stack.push(2).is_err() {
        return false;
    }
    if stack.push(0) != Err(LockFreeIndexStackPushError::AlreadyPresent) {
        return false;
    }
    if stack.push(4) != Err(LockFreeIndexStackPushError::OutOfRange) {
        return false;
    }
...
pub fn bounded_channel_static_smoke() -> bool {
    ...
    // Full
    if tx.send(99u64).is_ok() {
        return false;
    }

    for i in 0..cap {
        if rx.recv() != Some(i as u64) {
            return false;
        }
    }
