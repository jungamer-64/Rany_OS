...
impl KeyboardHandler {
    ...
    pub fn scancode_to_char(&self, scancode: u8, extended: bool) -> Option<char> {
        ...
        let c = match scancode {
            ...
            0x0B => if shifted { 0 as char } else { '0' }, // Shift+0 は割り当てなし（環境依存）
            0x0C => if shifted { '=' } else { '-' },
            ...
            0x27 => if shifted { '+' } else { ';' }, // JISの : キー
            0x28 => if shifted { '*' } else { ':' }, // JISの ] キー位置... ではなく : キー位置?
            // JIS配列:
            // 0x27 (US ;) -> ; / +
            // 0x28 (US ') -> : / *
            // 0x1A (US [) -> @ / `
            // 0x1B (US ]) -> [ / {
            // 0x2B (US \) -> ] / }
            0x29 => if shifted { 0 as char } else { 0 as char }, // Hankaku/Zenkaku (usually ignored or handled specially)
            ...
            0x73 => if shifted { '_' } else { '\\' }, // RO / Backslash (JIS only)
            0x7D => if shifted { '|' } else { '\\' }, // Yen / Pipe (JIS only)
            0x39 => ' ',
            0x0F => '\t',
            0x1C => '\n',
            _ => return None,
