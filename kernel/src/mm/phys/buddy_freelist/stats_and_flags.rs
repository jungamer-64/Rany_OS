...
impl LockedFreeListBuddyAllocator {
    ...
    pub fn allocate_4k_frame(&self) -> Option<PhysFrame<Size4KiB>> {
        self.0.lock().expect("lock poisoned").allocate_4k_frame()
    }

    /// 2MiBフレームを割り当て
    pub fn allocate_2m_frame(&self) -> Option<PhysFrame<Size2MiB>> {
        self.0.lock().expect("lock poisoned").allocate_2m_frame()
    }

    /// 1GiBフレームを割り当て
    pub fn allocate_1g_frame(&self) -> Option<PhysFrame<Size1GiB>> {
        self.0.lock().expect("lock poisoned").allocate_1g_frame()
    }
    ...
    pub fn free_count(&self) -> u64 {
        self.0.lock().expect("lock poisoned").free_count()
    }

    /// 統計: 総フレーム数
    pub fn total_count(&self) -> usize {
        self.0.lock().expect("lock poisoned").total_count()
    }

    /// 詳細統計情報を取得
    pub fn stats(&self) -> FreeListBuddyStats {
        self.0.lock().expect("lock poisoned").stats()
    }
}
