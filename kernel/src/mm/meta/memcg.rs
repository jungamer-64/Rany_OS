...
impl MemcgCounter {
    ...
    pub fn sub(&self, pages: u64) {
        self.current.fetch_sub(
            pages.min(self.current.load(Ordering::Relaxed)),
            Ordering::Relaxed,
        );
    }
...
impl MemcgManager {
    ...
    pub fn init(&self) {
        ...
        {
            // rootを作成
            let root = MemCgroup::new(MemcgId::ROOT, String::from("root"), None);
    ...
    /// 新しいCgroupを作成
    pub fn create(&self, name: String, parent_id: MemcgId) -> Result<MemcgId, MemcgError> {
        // 親の存在確認
        {
        ...
        log::debug!(
            "[Memcg] Created cgroup '{}' (id={}) under parent {}",
            name,
            new_id.as_u64(),
            parent_id.as_u64()
        );
