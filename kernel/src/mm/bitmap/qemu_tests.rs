...
pub fn basic_allocation_smoke() -> bool {
    ...
    if bitmap.free_count() != 1000 { return false; }
    ...
    if bitmap.free_count() != 999 { return false; }
    if bitmap.is_free(idx) { return false; }
...
pub fn mark_allocated_free_smoke() -> bool {
    ...
    if !bitmap.mark_allocated(50) { return false; }
    if bitmap.is_free(50) { return false; }
    if bitmap.mark_allocated(50) { return false; }

    if !bitmap.mark_free(50) { return false; }
    if !bitmap.is_free(50) { return false; }
    !bitmap.mark_free(50)
}
...
pub fn exhaustion_smoke() -> bool {
    ...
    if allocated.len() != 64 { return false; }
    if bitmap.free_count() != 0 { return false; }
    if bitmap.allocate_one().is_some() { return false; }
...
pub fn range_free_smoke() -> bool {
    ...
    if !bitmap.is_range_free(0, 64) { return false; }
    if !bitmap.is_range_free(100, 50) { return false; }

    bitmap.mark_allocated(110);
    !bitmap.is_range_free(100, 50)
        && bitmap.is_range_free(100, 10)
        && bitmap.is_range_free(111, 39)
}
...
pub fn claim_word_smoke() -> bool {
    ...
    if claimed != u64::MAX { return false; }
    if bitmap.free_count() != 64 { return false; }

    let claimed2 = bitmap.try_claim_word(0);
    if claimed2 != 0 { return false; }
...
pub fn last_word_partial_smoke() -> bool {
    ...
    if mask != (1u64 << 36) - 1 { return false; }
