...
impl PageReclaimController {
    ...
    pub(super) fn finalize_reclaim_success(&self, entry: &MglruEntry, node_idx: usize) {
        ...
        crate::mm::reclaim::workingset::workingset_evict(
            entry.frame,
            entry.generation,
    ...
    pub(super) fn reclaim_dirty_file_with_backing(
        ...
        match crate::mm::reclaim::async_swapout::try_enqueue_swapout(entry.frame, kind) {
            ...
            Err(crate::mm::reclaim::async_swapout::SwapError::AlreadyPending) => {
            ...
            | Err(crate::mm::reclaim::async_swapout::SwapError::NotSupported) => {
                ...
                {
                    crate::mm::meta::memcg::memcg_untrack_and_uncharge(entry.frame, count);
                    let _ = crate::mm::meta::frame_backing::untrack_frame_backing(entry.frame);
                    self.free_frame(entry.frame);
                    ReclaimOutcome::FreedNow
                } else {
    ...
    pub(super) fn try_async_swapout(
        ...
        entry: &MglruEntry,
        node_idx: usize,
        kind: crate::mm::reclaim::async_swapout::SwapKind,
    ) -> ReclaimOutcome {
        match crate::mm::reclaim::async_swapout::try_enqueue_swapout(entry.frame, kind) {
            Ok(_handle) => {
                self.enqueue_pending_async(entry, node_idx);
