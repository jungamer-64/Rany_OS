...
impl Drop for Wave7OverrideGuard {
    fn drop(&mut self) {
        crate::mm::reclaim::async_swapout::qemu_test_clear_enqueue_override();
        crate::mm::reclaim::page_reclaim::qemu_test_clear_writeback_overrides();
    }
}
...
pub fn wave7_file_backed_clean_reclaims_with_unsafe_disabled_smoke() -> bool {
    ...
    let ok = reclaimed == 1
        ...
        && !crate::mm::phys::buddy_allocator::is_frame_allocated(frame_idx.as_usize());
...
pub fn wave7_file_backed_dirty_reclaims_on_writeback_success_with_unsafe_disabled_smoke() -> bool {
    ...
    crate::mm::reclaim::async_swapout::qemu_test_set_enqueue_override(Some(
        crate::mm::reclaim::async_swapout::SwapError::QueueFull,
    ));
    crate::mm::reclaim::page_reclaim::qemu_test_set_sync_page_writeback_override(Some(true));
    ...
    let frame_idx = FrameIndex::from_phys_addr(frame.start_address().as_u64());
    crate::mm::meta::frame_backing::track_frame_backing(frame_idx, 0x10, 0);
    ...
    let ok = reclaimed == 1
        ...
        && !crate::mm::phys::buddy_allocator::is_frame_allocated(frame_idx.as_usize())
        && crate::mm::meta::frame_backing::get_frame_backing(frame_idx).is_none();
...
pub fn wave7_file_backed_dirty_requeues_on_writeback_failure_with_unsafe_disabled_smoke() -> bool {
    ...
    crate::mm::reclaim::page_reclaim::qemu_test_set_sync_page_writeback_override(Some(false));
    crate::mm::reclaim::page_reclaim::qemu_test_set_sync_all_writeback_override(Some(false));
    ...
    let frame_idx = FrameIndex::from_phys_addr(frame.start_address().as_u64());
    crate::mm::meta::frame_backing::track_frame_backing(frame_idx, 0x11, 1);
...
pub fn wave7_file_backed_dirty_without_backing_requeues_with_unsafe_disabled_smoke() -> bool {
    let _guard = init_wave7_overrides();
    crate::mm::reclaim::page_reclaim::qemu_test_set_sync_all_writeback_override(Some(true));
...
pub fn wave7_notsupported_file_dirty_falls_back_without_writeback_skipped_on_success_smoke() -> bool
{
    ...
    let frame_idx = FrameIndex::from_phys_addr(frame.start_address().as_u64());
    crate::mm::meta::frame_backing::track_frame_backing(frame_idx, 0x22, 2);
...
pub fn wave7_notsupported_file_dirty_requeues_and_counts_writeback_skipped_on_failure_smoke() -> bool
{
    ...
    let frame_idx = FrameIndex::from_phys_addr(frame.start_address().as_u64());
    crate::mm::meta::frame_backing::track_frame_backing(frame_idx, 0x33, 3);
...
pub fn wave7_queuefull_does_not_count_writeback_skipped_smoke() -> bool {
    ...
    let token_before = crate::mm::reclaim::async_swapout::token_count();
    crate::mm::reclaim::async_swapout::set_token_count(0);

    let Some(frame) = crate::mm::phys::frame_allocator::alloc_frame() else {
        crate::mm::reclaim::async_swapout::set_token_count(token_before);
        return true;
    };
    let frame_idx = FrameIndex::from_phys_addr(frame.start_address().as_u64());
    let mut entry = MglruEntry::new(frame_idx, PageType::Anonymous, 0);
