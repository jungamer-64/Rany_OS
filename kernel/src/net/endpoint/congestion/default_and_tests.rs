...
pub mod qemu_tests {
    ...
    pub fn cubic_fast_recovery_smoke() -> bool {
        ...
        cc.state() == CongestionState::FastRecovery && cc.base.ssthresh == 35000 && cc.w_max == 50000
    }
    ...
    pub fn variant_from_algorithm_smoke() -> bool {
        ...
        if nr.algorithm() != CongestionAlgorithm::NewReno || nr.cwnd() != INITIAL_WINDOW * DEFAULT_MSS {
            return false;
        }

        let cubic = CongestionControllerVariant::from_algorithm(CongestionAlgorithm::Cubic);
        if cubic.algorithm() != CongestionAlgorithm::Cubic || cubic.cwnd() != INITIAL_WINDOW * DEFAULT_MSS {
            return false;
        }
