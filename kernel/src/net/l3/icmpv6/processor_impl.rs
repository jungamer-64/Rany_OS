...
impl Icmpv6Processor {
    ...
    fn handle_echo_request_payload(
        ...
        let Ok(echo_data) = crate::net::payload::OwnedPayloadWindow::take_payload(payload, window)
        else {
            return Icmpv6Result::Error;
        };
        ...
        F: FnOnce(u8, u32, Ipv6Address, Ipv6Address, PacketPayload) -> Icmpv6Result,
    {
        ...
        if view.total_len() >= 48 {
            ...
            let Some(window) =
                crate::net::payload::PayloadWindowRequest::bounded_by(&payload, 8, quoted_len)
            else {
                return Icmpv6Result::Error;
            };
