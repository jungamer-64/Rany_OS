...
struct TlsEncryptedRecordBody {
    body: PayloadRange,
    ciphertext: MutablePayloadBounds,
    tag: PayloadRange,
}
...
impl TlsEncryptedRecordBody {
    fn from_record_body(
        ...
        let ciphertext = MutablePayloadBounds::checked(payload, body_offset, ciphertext_len)
            .ok_or(TlsError::DecodeError)?;
