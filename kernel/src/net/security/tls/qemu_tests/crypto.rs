...
fn chacha20_encrypt(key: &[u8; 32], nonce: &[u8; 12], counter: u32, data: &[u8]) -> Vec<u8> {
    let mut output = data.to_vec();
    chacha20_xor_in_place(key, nonce, counter, &mut output);
    output
}
...
fn chacha20_poly1305_encrypt(
    ...
    let mut tag = [0u8; 16];
    chacha20_poly1305_encrypt_in_place(key, nonce, aad, &mut ciphertext, &mut tag);
    (ciphertext, tag)
}
