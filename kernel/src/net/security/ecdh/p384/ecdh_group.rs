...
impl EcdhKeyPair {
    ...
    pub fn generate(group: EcdhGroup) -> Result<Self, EcdhError> {
        match group {
            ...
            EcdhGroup::Secp256r1 => {
                let mut sk_bytes = crate::net::security::tls::generate_random();
                ...
                while !crate::net::security::ecdh::scalar_is_valid(&sk_bytes) {
                    ...
                    sk_bytes = crate::net::security::tls::generate_random();
                }

                let pub_point = crate::net::security::ecdh::scalar_base_mul(&sk_bytes);
                let pk_bytes = crate::net::security::ecdh::encode_uncompressed_point(&pub_point)
...
pub mod qemu_tests {
    ...
    pub fn ecdh_p256_scalar_mul_base_smoke() -> bool {
        ...
        scalar_one[31] = 1;
        let result = crate::net::security::ecdh::scalar_base_mul(&scalar_one);
        !result.is_identity()
    }
}
