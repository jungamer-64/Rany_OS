...
//! # 統合ドメイン管理システム
//!
//! ドメイン（セル）の中核管理を担当するモジュール。
//!
//! ## 責務
//!
//! - `DomainId`, `DomainState`, `Domain` 構造体の定義
//! - ドメインレジストリの管理（作成・検索・一覧・状態変更）
//! - Exchange Heap との連携（`register_heap_object` / `unregister_heap_object`）
//! - ドメインの終了・パニック処理（`terminate_domain`, `handle_domain_panic`）
//! - CPU クォータ制御（`cpu_tick_domain`, `CpuQuotaAction`）
//!
//! ## 関連モジュール
//!
//! - `domain/quota` — リソースクォータ定義（メモリ・CPU・I/O制限）
//! - `domain/lifecycle` — 高レベルライフサイクル操作（タスクラップ・依存関係）。
//!   `terminate_domain()` 等は本モジュールの同名関数に委譲する。
