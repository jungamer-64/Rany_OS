...
pub(crate) fn encode_alphanumeric_v1_l(data: &[u8]) -> Option<[u8; TOTAL_CODEWORDS]> {
    ...
    if !append_bits(&mut bit_stream, &mut bit_idx, 0b0010, 4) {
        return None;
    }
    if !append_bits(&mut bit_stream, &mut bit_idx, data.len() as u32, 9) {
        return None;
    }
...
pub(crate) fn encode_alnum_pairs(
    data: &[u8],
    bit_stream: &mut [u8],
    bit_idx: &mut usize,
) -> Option<()> {
    ...
    while i < data.len() {
        ...
        if i + 1 < data.len() {
            ...
            i += 2;
            if !append_bits(bit_stream, bit_idx, (val1 as u32) * 45 + (val2 as u32), 11) {
                return None;
            }
        } else {
            i += 1;
            if !append_bits(bit_stream, bit_idx, val1 as u32, 6) {
                return None;
            }
        }
    }
...
pub(crate) fn pad_qr_bitstream(bit_stream: &mut [u8], bit_idx: &mut usize) -> Option<()> {
    if *bit_idx > DATA_PAYLOAD_BITS {
        return None;
    }

    let term_len = core::cmp::min(4, DATA_PAYLOAD_BITS - *bit_idx);
    if !append_bits(bit_stream, bit_idx, 0, term_len) {
        return None;
    }

    if *bit_idx % 8 != 0 {
        let pad = 8 - (*bit_idx % 8);
        if !append_bits(bit_stream, bit_idx, 0, pad) {
            return None;
        }
    }
