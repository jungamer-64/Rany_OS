...
pub fn test_large_allocation() -> TestResult {
    ...
    for (i, &byte) in data.iter().enumerate() {
        if byte != (i & 0xFF) as u8 {
            errors += 1;
            if errors > 10 {
                return TestResult::Failed(String::from(
                    "Too many corruption errors in large allocation",
                ));
            }
        }
    }
...
pub fn test_vec_growth() -> TestResult {
    ...
    for (i, &val) in vec.iter().enumerate() {
        if val != i as u32 {
            return TestResult::Failed(alloc::format!(
                "Vec corruption at index {}: expected {}, got {}",
                i,
                i,
                val
            ));
    ...
    if vec.capacity() < vec.len() {
        return TestResult::Failed(String::from(
            "shrink_to_fit made capacity smaller than length",
        ));
    }
...
pub fn test_memory_alignment() -> TestResult {
    ...
    if ptr % 64 != 0 {
        return TestResult::Failed(alloc::format!(
            "64-byte aligned struct not aligned: address 0x{:x}",
            ptr
        ));
    }
    ...
    if ptr % 16 != 0 {
        return TestResult::Failed(alloc::format!(
            "16-byte aligned struct not aligned: address 0x{:x}",
            ptr
        ));
    }
