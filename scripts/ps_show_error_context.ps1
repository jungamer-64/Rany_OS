...
if ($errors.Count -eq 0) { Write-Output 'No parse errors'; exit 0 }
foreach ($e in $errors) {
    $start = [math]::Max(1, $e.Extent.StartLineNumber - 5)
    $end = [math]::Min(($content -split "`n").Count, $e.Extent.EndLineNumber + 5)
    Write-Output "Error: $($e.Message) at $($e.Extent.StartLineNumber):$($e.Extent.StartColumnNumber)"
    $allLines = [regex]::Split($content, '\r?\n')
    $lines = $allLines[$start-1..$end-1]
    for ($i=$start;$i -le $end;$i++) {
        $ln = $lines[$i-$start]
        Write-Output ("{0,4}: {1}" -f $i, $ln)
