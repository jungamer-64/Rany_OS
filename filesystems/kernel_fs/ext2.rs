...
//! # ⚠ 非推奨 (POSIX Legacy)
//!
//! ext2はLinuxカーネル固有のファイルシステムであり、Exokernelの設計では
//! ファイルシステムはユーザー空間ライブラリ（libFS）として実装すべきです。
//!
//! ## 移行先
//! - カーネル内FSドライバ → `filesystems/` クレートのユーザー空間実装
//! - ブロックデバイスアクセスは `fs::block::ZeroCopyBlockDevice` 経由
//! - FAT32は `filesystems/fat32` として既に外部化済み
//!
//! ## 機能
//! - スーパーブロック解析
//! - ブロックグループ管理
...
impl FileSystem for Ext2FileSystem {
    ...
    fn sync(&self) -> FsResult<()> {
        // ext2キャッシュをフラッシュ
        //
        // Note: 現在の実装では、inode/ブロック書き込み時に即座にディスクに書き込むため、
        // 明示的なダーティトラッキングは行っていない。デバイスレベルのflush()のみ実行。
        //
        // 将来の最適化:
