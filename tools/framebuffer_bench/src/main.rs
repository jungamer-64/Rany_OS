...
use rany_os::graphics::framebuffer::{force_packer_mode, current_packer_mode};
...
fn bench_draw_image_with_criterion(c: &mut Criterion) {
    ...
    c.bench_function("fill_rect_full", |b| b.iter(|| {
        fb_bgra.fill_rect(rany_os::graphics::Rect::new(0, 0, width, height), Color::BLUE);
    }));
    ...
    c.bench_function("large_buffer_fill_8k", |b| b.iter(|| {
    ...
    c.bench_function("draw_image_rgb565", |b| b.iter(|| {
    ...
    c.bench_function("write_bytes_small_16", |b| b.iter(|| {
        fb_bgr24_mmio.bench_write_bytes_mmio(0, 0, &data_small);
    }));
    c.bench_function("write_bytes_large_16k", |b| b.iter(|| {
    ...
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    {
        // Scalar (baseline)
        force_packer_mode(1);
        c.bench_function("pack_rgba_forced_scalar", |b| b.iter(|| {
            Framebuffer::pack_rgba_to_bgra(&src_pack, &mut dst_pack2);
        }));
        ...
        if std::is_x86_feature_detected!("avx2") {
            force_packer_mode(3);
            c.bench_function("pack_rgba_forced_avx2", |b| b.iter(|| {
                Framebuffer::pack_rgba_to_bgra(&src_pack, &mut dst_pack2);
            }));
        }
    }
