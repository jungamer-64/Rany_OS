...
fn bench_draw_text_32bit_mmio_batch(c: &mut Criterion) {
    ...
    let text = "The quick brown fox jumps over the lazy dog. 0123456789!@#$%^&*()";
    c.bench_function("draw_text_32bit_mmio_batch", |b| {
        b.iter(|| {
            fb.draw_text(100, 100, text, Color::WHITE, Color::BLACK);
        })
    });
...
fn bench_draw_text_32bit_mmio_per_glyph_fenced(c: &mut Criterion) {
    ...
    let text = "The quick brown fox jumps over the lazy dog. 0123456789!@#$%^&*()";
    c.bench_function("draw_text_32bit_mmio_per_glyph_fenced", |b| {
        b.iter(|| {
...
fn bench_fill_rect_full_mmio_batch(c: &mut Criterion) {
    ...
    c.bench_function("fill_rect_full_mmio_batch", |b| {
        b.iter(|| {
            fb.fill_rect(
                rany_os::graphics::Rect::new(0, 0, width, height),
                Color::BLUE,
...
fn bench_fill_rect_full_mmio_per_row_fenced(c: &mut Criterion) {
    ...
    c.bench_function("fill_rect_full_mmio_per_row_fenced", |b| {
        b.iter(|| {
            fb.bench_fill_rect_per_row_fenced(
                rany_os::graphics::Rect::new(0, 0, width, height),
                Color::BLUE,
            );
        })
