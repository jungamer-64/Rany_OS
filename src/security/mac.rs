...
pub enum MacError {
    ...
    CategoryDenied { missing: Vec<SecurityCategory> },
    /// Type transition not allowed
    TypeTransitionDenied { from: String, to: String },
    /// Policy not loaded
    NoPolicyLoaded,
}
...
impl MacPolicy {
    ...
    pub fn check_domain_access(
        ...
        let subject = self
            .get_domain_context(subject_domain)
            .unwrap_or(SecurityContext::public());
        let object = self
            .get_domain_context(object_domain)
            .unwrap_or(SecurityContext::public());
