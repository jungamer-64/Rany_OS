...
impl From<Fat32Error> for FsError {
    fn from(err: Fat32Error) -> Self {
        match err {
            Fat32Error::InvalidBootSector { .. } => FsError::InvalidArgument,
            Fat32Error::InvalidCluster { .. } => FsError::CorruptedFs,
            Fat32Error::ClusterChainLoop { .. } => FsError::CorruptedFs,
            Fat32Error::PathTooLong { .. } => FsError::NameTooLong,
...
impl DirEntryRaw {
    ...
    pub fn as_bytes(&self) -> &[u8] {
        // SAFETY: 上記の厳格な前提条件を満たしている
        // DirEntryRawはPOD型であり、全フィールドがプリミティブバイト配列
        unsafe {
...
impl Fat32Inode {
    ...
    /// # Errors
    /// 
    /// - `FsError::NotDirectory` - このinodeがディレクトリでない場合
    /// - `FsError::IoError` - ディスク読み取りエラー
    /// - `FsError::CorruptedFs` - クラスタチェーンが破損している場合
    /// 
    /// # Performance
    /// 
