...
pub fn pci_write(bus: u8, device: u8, function: u8, offset: u8, value: u32) {
    PCI_CONFIG.lock().write(bus, device, function, offset, value);
}
...
impl PciDevice {
        ...
        while i < max_bars {
            ...
            if (bar_value & 1) == 0 {
                ...
                if is_64bit && i + 1 < max_bars {
                    ...
                    if size_high != 0 {
                        let full_mask = ((size_high as u64) << 32) | (size_mask as u64 & 0xFFFFFFF0);
    ...
    pub fn enable_bus_master(&self) {
        ...
        pci_write(self.bus, self.device, self.function, 0x04, 
...
pub fn find_by_class(class: u8, subclass: u8) -> Vec<PciDevice> {
    PCI_BUS.lock()
        .as_ref()
        .map(|b| b.find_by_class(class, subclass).into_iter().cloned().collect())
        .unwrap_or_default()
}
