...
use crate::io::pci::{pci_read, pci_write, pci_read16, pci_read8, PciDevice};
use spin::Mutex;
use alloc::vec::Vec;
...
impl MsiCapability {
    ...
    pub fn probe(device: &PciDevice) -> Option<Self> {
        ...
        let message_control = pci_read16(device.bus, device.device, device.function, cap_offset + 2);
    ...
    pub fn enable(&self, config: &MsiConfig) {
        ...
        pci_write(self.bus, self.device, self.function, 
                  self.cap_offset + 4, address as u32);
        
        if self.supports_64bit {
            // Write message address (high)
            pci_write(self.bus, self.device, self.function,
                      self.cap_offset + 8, (address >> 32) as u32);
            // Write message data
            pci_write(self.bus, self.device, self.function,
                      self.cap_offset + 12, data);
        } else {
            // Write message data
            pci_write(self.bus, self.device, self.function,
                      self.cap_offset + 8, data);
        }
        
        // Enable MSI
        let mut message_control = pci_read16(self.bus, self.device, self.function, 
                                              self.cap_offset + 2);
        ...
        pci_write(self.bus, self.device, self.function, self.cap_offset,
    ...
    pub fn is_enabled(&self) -> bool {
        let message_control = pci_read16(self.bus, self.device, self.function,
                                          self.cap_offset + 2);
    ...
    pub fn mask_vector(&self, vector: u8) {
        ...
        pci_write(self.bus, self.device, self.function, mask_offset,
                  mask | (1u32 << vector));
    }
...
impl MsixCapability {
    ...
    pub fn probe(device: &PciDevice) -> Option<Self> {
        ...
        let message_control = pci_read16(device.bus, device.device, device.function, 
                                          cap_offset + 2);
    ...
    pub unsafe fn configure_vector(&self, table_base: *mut MsixTableEntry, 
                                   vector: u16, config: &MsiConfig) {
...
impl InterruptAllocator {
    ...
    fn allocate(&mut self, bus: u8, device: u8, function: u8) -> Option<u8> {
        if self.next_vector >= 224 { // Reserve 224-255 for system use
            return None;
        }
...
pub fn allocate_vector(device: &PciDevice) -> Option<u8> {
    INTERRUPT_ALLOCATOR.lock().allocate(device.bus, device.device, device.function)
}

/// Allocate multiple contiguous interrupt vectors
pub fn allocate_vectors(device: &PciDevice, count: u8) -> Option<u8> {
    INTERRUPT_ALLOCATOR.lock().allocate_range(device.bus, device.device, device.function, count)
}
...
pub fn setup_msix(device: &PciDevice, apic_id: u8, num_vectors: u16) -> Option<(MsixCapability, u8)> {
