...
impl SystemIntegration {
    ...
    fn integrate_acpi(&mut self) -> Result<(), IntegrationError> {
        ...
        self.log(&alloc::format!("  Found {} processor(s)", local_apics.len()));
        self.log(&alloc::format!("  Found {} I/O APIC(s)", io_apics.len()));
        self.log(&alloc::format!("  Found {} PCIe ECAM region(s)", pcie_ecam.len()));
        
        // Store APIC information for interrupt routing
        for apic in &io_apics {
            self.interrupt_router.add_io_apic(apic.id, apic.address, apic.gsi_base);
        }
        
        // Store interrupt overrides
        let overrides = crate::io::acpi::interrupt_overrides();
        for ovr in &overrides {
            self.interrupt_router.add_override(ovr.source, ovr.gsi, ovr.polarity, ovr.trigger_mode);
        }
    ...
    fn integrate_interrupts(&mut self) -> Result<(), IntegrationError> {
        ...
        self.log(&alloc::format!("  Configured {} interrupt route(s)", routes));
        
        // Allocate MSI vectors for capable devices
        let msi_devices: Vec<_> = self.device_manager.get_msi_capable()
            .into_iter()
            .map(|d| (d.id, d.name.clone(), d.pci_location))
            .collect();
        self.log(&alloc::format!("  {} device(s) support MSI/MSI-X", msi_devices.len()));
        ...
        for pci_dev in &pci_devices {
            for (dev_id, dev_name, pci_loc) in &msi_devices {
                if pci_loc.map(|l| l.bus == pci_dev.bus && l.device == pci_dev.device && l.function == pci_dev.function).unwrap_or(false)
                {
                    if let Some(vector) = crate::io::msi::allocate_vector(pci_dev) {
                        self.interrupt_router.add_msi_route(*dev_id, vector);
                        self.log(&alloc::format!("    Device {} -> vector {}", dev_name, vector));
                    }
                    break;
                }
            }
    ...
    fn integrate_devices(&mut self) -> Result<(), IntegrationError> {
        ...
        for dev in virtio_devices {
            match dev.device_id {
                0x1001 | 0x1042 => {
                    // VirtIO Block Device
                    self.log(&alloc::format!("  Initializing VirtIO-Blk at {:02x}:{:02x}.{}",
                ...
                0x1000 | 0x1041 => {
                    ...
                    self.log(&alloc::format!("  Initializing VirtIO-Net at {:02x}:{:02x}.{}",
                        dev.bus, dev.device, dev.function));
                    dev.enable_bus_master();
                    dev.enable_memory_space();
                }
        ...
        for dev in nvme_devices {
            self.log(&alloc::format!("  Initializing NVMe controller at {:02x}:{:02x}.{}",
                dev.bus, dev.device, dev.function));
            dev.enable_bus_master();
            dev.enable_memory_space();
        }
    ...
    fn integrate_security(&mut self) -> Result<(), IntegrationError> {
        ...
        self.log(&alloc::format!("  Bound {} device(s) to security contexts", device_count));
