...
impl SriovController {
    pub fn new(config: &'static PcieConfig, pf_bdf: PcieBdf) -> PcieResult<Self> {
        // SR-IOV拡張ケイパビリティを検索
        let offset = config.find_ext_capability(pf_bdf, ext_cap_id::SRIOV)
    ...
    fn read_capability(config: &PcieConfig, bdf: PcieBdf, offset: u16) -> PcieResult<SriovCapability> {
        let total_vfs = config.read16(bdf, offset + 0x0E)
            .ok_or(PcieError::ConfigError)?;
        let num_vfs = config.read16(bdf, offset + 0x10)
            .ok_or(PcieError::ConfigError)?;
        let first_vf_offset = config.read16(bdf, offset + 0x14)
            .ok_or(PcieError::ConfigError)?;
        let vf_stride = config.read16(bdf, offset + 0x16)
            .ok_or(PcieError::ConfigError)?;
        let vf_device_id = config.read16(bdf, offset + 0x1A)
            .ok_or(PcieError::ConfigError)?;
        let supported_page_sizes = config.read32(bdf, offset + 0x1C)
            .ok_or(PcieError::ConfigError)?;
        let system_page_size = config.read32(bdf, offset + 0x20)
            .ok_or(PcieError::ConfigError)?;
    ...
    pub fn enable_vfs(&self, num_vfs: u16) -> PcieResult<()> {
        ...
        // NumVFsを設定
        self.config.write16(self.pf_bdf, offset + 0x10, num_vfs)
            .ok_or(PcieError::ConfigError)?;
        
        // VF Enableビットをセット
        let control = self.config.read16(self.pf_bdf, offset + 0x08)
            .ok_or(PcieError::ConfigError)?;
        self.config.write16(self.pf_bdf, offset + 0x08, control | 0x01)
            .ok_or(PcieError::ConfigError)?;
        
        // MSE (Memory Space Enable)を設定
        self.config.write16(self.pf_bdf, offset + 0x08, control | 0x09)
            .ok_or(PcieError::ConfigError)?;
    ...
    pub fn disable_vfs(&self) -> PcieResult<()> {
        ...
        // NumVFsを0に
        self.config.write16(self.pf_bdf, offset + 0x10, 0)
            .ok_or(PcieError::ConfigError)?;
...
impl AerController {
    pub fn new(config: &'static PcieConfig, bdf: PcieBdf) -> PcieResult<Self> {
        let offset = config.find_ext_capability(bdf, ext_cap_id::AER)
            .ok_or(PcieError::CapabilityNotFound)?;
    ...
    pub fn read_correctable_errors(&self) -> PcieResult<CorrectableErrors> {
        let cap = self.capability.as_ref().ok_or(PcieError::CapabilityNotFound)?;
        let status = self.config.read32(self.bdf, cap.offset + 0x10)
            .ok_or(PcieError::ConfigError)?;
    ...
    pub fn read_uncorrectable_errors(&self) -> PcieResult<UncorrectableErrors> {
        let cap = self.capability.as_ref().ok_or(PcieError::CapabilityNotFound)?;
        let status = self.config.read32(self.bdf, cap.offset + 0x04)
            .ok_or(PcieError::ConfigError)?;
    ...
    pub fn clear_correctable_errors(&self) -> PcieResult<()> {
        ...
        // Write-1-to-clear
        self.config.write32(self.bdf, cap.offset + 0x10, status)
            .ok_or(PcieError::ConfigError)?;
    ...
    pub fn clear_uncorrectable_errors(&self) -> PcieResult<()> {
        ...
        // 重大度をチェック
        let severity = self.config.read32(self.bdf, cap.offset + 0x0C)
            .ok_or(PcieError::ConfigError)?;
        let fatal = status & severity;
        
        // Write-1-to-clear
        self.config.write32(self.bdf, cap.offset + 0x04, status)
            .ok_or(PcieError::ConfigError)?;
    ...
    pub fn set_correctable_mask(&self, mask: u32) -> PcieResult<()> {
        let cap = self.capability.as_ref().ok_or(PcieError::CapabilityNotFound)?;
        self.config.write32(self.bdf, cap.offset + 0x14, mask)
            .ok_or(PcieError::ConfigError)
    }
    
    pub fn set_uncorrectable_mask(&self, mask: u32) -> PcieResult<()> {
        let cap = self.capability.as_ref().ok_or(PcieError::CapabilityNotFound)?;
        self.config.write32(self.bdf, cap.offset + 0x08, mask)
            .ok_or(PcieError::ConfigError)
    }
...
impl PciePowerManager {
    ...
    pub fn current_state(&self) -> PcieResult<PciePowerState> {
        ...
        let pmcsr = self.config.read16(self.bdf, offset as u16 + 4)
            .ok_or(PcieError::ConfigError)?;
    ...
    pub fn set_state(&self, state: PciePowerState) -> PcieResult<()> {
        ...
        // サポートされる状態をチェック
        let pmcap = self.config.read16(self.bdf, offset as u16 + 2)
            .ok_or(PcieError::ConfigError)?;
        ...
        self.config.write16(self.bdf, offset as u16 + 4, new_pmcsr)
            .ok_or(PcieError::ConfigError)
    }
    ...
    pub fn enable_pme(&self) -> PcieResult<()> {
        ...
        // PME_En ビットをセット
        self.config.write16(self.bdf, offset as u16 + 4, pmcsr | (1 << 8))
            .ok_or(PcieError::ConfigError)
    }
    ...
    pub fn clear_pme_status(&self) -> PcieResult<()> {
        ...
        self.config.write16(self.bdf, offset as u16 + 4, pmcsr | (1 << 15))
            .ok_or(PcieError::ConfigError)
    }
}
...
impl MsixController {
    pub fn new(config: &'static PcieConfig, bdf: PcieBdf) -> PcieResult<Self> {
        let offset = config.find_capability(bdf, cap_id::MSIX)
            .ok_or(PcieError::CapabilityNotFound)?;
        
        // Message Control を読み取り
        let msg_ctrl = config.read16(bdf, offset as u16 + 2)
            .ok_or(PcieError::ConfigError)?;
        let table_size = (msg_ctrl & 0x07FF) + 1;
        
        // Table Offset/BIR
        let table_offset_bir = config.read32(bdf, offset as u16 + 4)
            .ok_or(PcieError::ConfigError)?;
        let table_bir = (table_offset_bir & 0x07) as u8;
        let table_offset = table_offset_bir & !0x07;
        
        // PBA Offset/BIR
        let pba_offset_bir = config.read32(bdf, offset as u16 + 8)
            .ok_or(PcieError::ConfigError)?;
    ...
    pub fn enable(&self) -> PcieResult<()> {
        ...
        let msg_ctrl = self.config.read16(self.bdf, offset as u16 + 2)
            .ok_or(PcieError::ConfigError)?;
    ...
    pub fn disable(&self) -> PcieResult<()> {
        ...
        self.config.write16(self.bdf, offset as u16 + 2, msg_ctrl & !0x8000)
            .ok_or(PcieError::ConfigError)
    }
...
impl HotPlugController {
    pub fn new(config: &'static PcieConfig, bdf: PcieBdf) -> PcieResult<Self> {
        let offset = config.find_capability(bdf, cap_id::PCIE)
            .ok_or(PcieError::CapabilityNotFound)?;
        
        // PCIe Capabilities を読み取り
        let pcie_caps = config.read16(bdf, offset as u16 + 2)
            .ok_or(PcieError::ConfigError)?;
    ...
    pub fn slot_status(&self) -> PcieResult<u16> {
        ...
        self.config.read16(self.bdf, offset as u16 + 0x1A)
            .ok_or(PcieError::ConfigError)
    }
    ...
    pub fn power_on(&self) -> PcieResult<()> {
        ...
        let offset = self.pcie_offset.ok_or(PcieError::CapabilityNotFound)?;
        let slot_ctrl = self.config.read16(self.bdf, offset as u16 + 0x18)
            .ok_or(PcieError::ConfigError)?;
        
        // Power Controller Control = 0 (Power On)
        self.config.write16(self.bdf, offset as u16 + 0x18, slot_ctrl & !0x0400)
            .ok_or(PcieError::ConfigError)
    }
    ...
    pub fn clear_events(&self) -> PcieResult<()> {
        ...
        // Write-1-to-clear
        self.config.write16(self.bdf, offset as u16 + 0x1A, status)
            .ok_or(PcieError::ConfigError)
    }
}
...
pub fn init(base_addr: u64) -> PcieResult<()> {
    let config = PCIE_CONFIG.call_once(|| {
        PcieConfig::new(base_addr, 0, 0, 255)
    });
