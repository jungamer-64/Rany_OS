...
impl DnsCache {
    ...
    pub fn lookup(&self, name: &str, current_tick: u64) -> Option<&DnsCacheEntry> {
        self.entries.get(name).filter(|entry| !entry.is_expired(current_tick, self.tick_rate))
    }
    ...
    pub fn insert(&mut self, name: String, records: Vec<DnsRecord>, current_tick: u64) {
        ...
        self.entries.insert(name, DnsCacheEntry {
            records,
    ...
    pub fn cleanup(&mut self, current_tick: u64) {
        self.entries.retain(|_, entry| !entry.is_expired(current_tick, self.tick_rate));
    }
...
impl DnsClient {
    ...
    /// DNSクエリパケットを構築
    pub fn build_query(&self, buffer: &mut [u8], name: &str, qtype: DnsQueryType) -> Result<usize, &'static str> {
    ...
    /// DNS応答を解析
    pub fn parse_response(&self, data: &[u8], current_tick: u64) -> Result<Vec<DnsRecord>, DnsResponseCode> {
        ...
        for _ in 0..acount {
            ...
            let record_data = match DnsQueryType::from_u16(rtype) {
                ...
                Some(DnsQueryType::TXT) => {
                    ...
                    if !rdata.is_empty() {
                        ...
                        if txt_len < rdlength {
                            DnsRecordData::TXT(String::from_utf8_lossy(&rdata[1..1+txt_len]).into_owned())
                        } else {
                            DnsRecordData::Raw(rdata.to_vec())
                        }
                    } else {
            ...
            records.push(DnsRecord {
                ...
                rclass: if rclass == 1 { DnsQueryClass::IN } else { DnsQueryClass::IN },
                ttl,
                data: record_data,
            });
        }
    ...
    /// ドメイン名を解析 (圧縮対応)
    fn parse_name(&self, data: &[u8], mut offset: usize) -> Result<(String, usize), DnsResponseCode> {
...
pub fn resolve_cached(name: &str, current_tick: u64) -> Option<Ipv4Address> {
    DNS_CLIENT.lock().as_ref().and_then(|c| c.resolve_cached(name, current_tick))
}
