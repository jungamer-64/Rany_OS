...
use super::ipv4::{Ipv4Address, IpProtocol, pseudo_header_checksum, data_checksum};
...
impl<'a> UdpPacket<'a> {
    ...
    pub fn header(&self) -> &UdpHeader {
        ...
        unsafe {
            &*(self.data.as_ptr() as *const UdpHeader)
        }
    }
...
impl<'a> UdpPacketMut<'a> {
    ...
    pub fn header_mut(&mut self) -> &mut UdpHeader {
        ...
        unsafe {
            &mut *(self.buffer.as_mut_ptr() as *mut UdpHeader)
        }
    }
    ...
    pub fn write_payload(&mut self, data: &[u8]) -> usize {
        ...
        self.buffer[UdpHeader::SIZE..UdpHeader::SIZE + len]
...
impl UdpProcessor {
    ...
    pub fn process(
        ...
        if !packet.verify_checksum(src_ip, dst_ip) {
            self.sockets.stats.checksum_errors.fetch_add(1, Ordering::Relaxed);
...
mod tests {
    ...
    fn test_udp_packet() {
        ...
        let len = UdpProcessor::build_packet(
            &mut buffer,
            src_ip,
            12345,
            dst_ip,
