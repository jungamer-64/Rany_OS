...
const AP_BOOT_PAGE_SIZE: u64 = 4096;

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct ApBootLayout {
    ap_count: u16,
    stack_count: u16,
...
impl ApBootLayout {
    ...
    pub const fn ap_count(&self) -> u16 {
        self.ap_count
    }

    pub const fn stack_count(&self) -> u16 {
        self.stack_count
    }

    pub const fn trampoline_base(&self) -> TrampolinePhysAddr {
        self.trampoline_base
    }

    pub const fn trampoline_size(&self) -> u64 {
    ...
    pub fn into_boot_info(self) -> ApBootInfo {
        ApBootInfo {
            ...
            _reserved2: [0; 4],
            trampoline_addr: self.trampoline_base.as_u64(),
    ...
    pub fn boot_capacity(&self) -> usize {
        usize::from(self.ap_count).min(usize::from(self.stack_count))
    }

    pub fn stack_region_bytes(&self) -> Option<u64> {
        let bytes = u64::from(self.stack_count).checked_mul(self.stack_size)?;
        (bytes > 0).then_some(bytes)
    }

    pub fn trampoline_range(&self) -> Option<(u64, u64)> {
        (self.trampoline_size > 0).then_some((self.trampoline_base.as_u64(), self.trampoline_size))
    }

    pub fn stack_region_range(&self) -> Option<(u64, u64)> {
        self.stack_region_bytes()
            .map(|size| (self.stack_base, size))
    }

    pub fn stack_base_for(&self, ap_index: usize) -> Option<u64> {
        if ap_index >= usize::from(self.stack_count) {
            return None;
        }

        let offset = u64::try_from(ap_index).ok()?.checked_mul(self.stack_size)?;
        self.stack_base.checked_add(offset)
    }

    pub fn stack_top_for(&self, ap_index: usize) -> Option<u64> {
        self.stack_base_for(ap_index)?.checked_add(self.stack_size)
    }
}

impl ApBootInfo {
    pub fn layout(&self) -> Result<ApBootLayout, &'static str> {
        if self.trampoline_addr == 0 {
            return Err("missing AP trampoline allocation");
        }

        let trampoline_base = TrampolinePhysAddr::new(self.trampoline_addr)?;
        if (self.flags & ApBootFlags::TRAMPOLINE_READY) == 0 {
            return Err("shared AP trampoline is not marked ready");
        }
        if self.trampoline_layout_version != LAYOUT_VERSION {
            return Err("shared AP trampoline layout version mismatch");
        }
        if self.trampoline_mailbox_offset != MAILBOX_OFFSET as u32 {
    ...
    pub fn trampoline_range(&self) -> Option<(u64, u64)> {
        if self.trampoline_addr == 0 || self.trampoline_size == 0 {
            return None;
        }

        Some((self.trampoline_addr, self.trampoline_size))
    }
